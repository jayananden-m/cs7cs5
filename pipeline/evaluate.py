...
def evaluate(agent_name, model_path, n_episodes, gui):
    ...
    for ep in range(1, n_episodes + 1):
        ...
        print(
            ...
            f"rmse={rmse_gap:.2f} | |Δv|={mean_relv:.3f} | survived={survived}"
        )

        if rps > best_rps:
            best_rps = rps
            best_ep = ep
            best_traj = {"gaps": gap_arr, "rel_vs": relv_arr, "speeds": spd_arr, "desired": desired}
    ...
    # Save best-episode gap trajectory CSV
...
def _save_trajectory(agent_name, traj):
    ...
    with open(path, "w", newline="") as f:
        fieldnames = ["step"] + [f"agent{i}_gap" for i in range(N)] + \
...
def _print_summary(agent_name, records):
    ...
    print(f"  RMSE gap error:  {rmse_gap:.2f} m")
